...
function _cb_post(et, ctx){
  ...
  if (perf_enable){
    ...
    var perf = E.perf_stat[name]||(E.perf_stat[name] = {ms: 0, n: 0, max: 0});
    if (perf.max<ms)
      perf.max = ms;
    perf.ms += ms;
    perf.n++;
...
function Etask(opt, states){
  ...
  for (var i=0; i<states.length; i++){
    ...
    var state = {f: pstate, label: t.label, try_catch: t.try_catch,
      catch: t.catch, finally: t.finally, cancel: t.cancel, sig: undefined};
...
E.prototype._set_retval = function(ret){
  if (ret===this.retval && !this.error); // fast-path retval already set
    ...
    [this.retval, this.error] = [ret, undefined];
  else if (ret instanceof Etask){
    if (ret.tm_completed)
      [this.retval, this.error] = [ret.retval, ret.error];
  } else if (ret instanceof Etask_err)
    [this.retval, this.error] = [undefined, ret.error];
  else if (typeof ret.then=='function'); // promise
  // generator
  else if (typeof ret.next=='function' && typeof ret.throw=='function');
  else
...
E.prototype.finally = function(cb){ this.prependListener('finally', cb); };
E.prototype.goto_fn = function(name){ return this.goto.bind(this, name); };
...
E.prototype.continue_fn = function(){ return this.continue.bind(this); };
E.continue_depth = 0;
...
Etask_wait.prototype.throw_fn = function(){ return this.throw.bind(this); };
...
E.prototype.throw = function(err){ return this.continue(E.err(err)); };
...
E.for = function(cond, inc, opt, states){
  ...
  return etask({name: 'for', cancel: true, init: opt.init_parent},
      [function loop(){ return !cond || cond.call(this);
...
E.all_limit = function(limit, arr_iter, cb){
  ...
  }, function done(){ return this.wait_child('all'); }]);
};
...
E.nfn_apply = function(opt, func, _this, args){
  ...
  if (typeof opt=='function' || typeof func=='string')
    [args, _this, func, opt] = [_this, func, opt, _opt];
  else
    opt = assign(_opt, opt);
...
E.interval = function(opt, states){
  if (typeof opt=='number')
    opt = {ms: opt};
  if (opt.mode=='fixed')
    return E.for(null, function(){ return etask.sleep(opt.ms); }, states);
