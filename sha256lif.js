/*!
 * sha256lif.js - SHA256LIF implementation for bcrypto
...
class SHA256LIF {
  ...
  static hash() {
    return new SHA256LIF();
  }

  /*
  static hmac() {
    return new HMAC(SHA256LIF, 64);
  }
  */

  static digest(data) {
    return SHA256LIF.ctx.init().update(data).final();
  }

  static root(left, right) {
    assert(Buffer.isBuffer(left) && left.length === 32);
    assert(Buffer.isBuffer(right) && right.length === 32);
    return SHA256LIF.ctx.init().update(left).update(right).final();
  }
  ...
  static mac(data, key) {
    return SHA256LIF.hmac().init(key).update(data).final();
  }
}
...
SHA256LIF.native = 0;
SHA256LIF.id = 'SHA256LIF';
SHA256LIF.size = 32;
SHA256LIF.bits = 256;
SHA256LIF.blockSize = 64;
SHA256LIF.zero = Buffer.alloc(32, 0x00);
SHA256LIF.ctx = new SHA256LIF();
